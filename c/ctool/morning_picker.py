...
If SMTP_USER/SMTP_PASSWORD/TO_EMAIL env vars are set, emails the results.
"""
...
def send_email(top3):
    ...
    weekday = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"][datetime.datetime.now().weekday()]
...
<tr>
  ...
  <td style="padding:8px 12px;border:1px solid #ddd;font-weight:bold">{item['code']}</td>
  <td style="padding:8px 12px;border:1px solid #ddd;text-align:right">{item['score']:.0f}/100</td>
  <td style="padding:8px 12px;border:1px solid #ddd;text-align:right">¥{item['price']:.2f}</td>
  <td style="padding:8px 12px;border:1px solid #ddd;font-size:12px">{signals}</td>
</tr>"""

    html = f"""<html><head><meta charset="UTF-8"></head><body style="font-family:Arial,sans-serif">
<h2>Pre-Break Top 3 Picks — {date_str} ({weekday})</h2>
<p>Scored {len(top3)} candidates from 948 stocks on 5 pre-break signals (RSI, volatility, MA20 distance, momentum, price range).</p>
...
<tr style="background:#f5f5f5">
  ...
  <th style="padding:8px 12px;border:1px solid #ddd">Signals</th>
</tr>
{rows}
</table>
<p style="color:gray;font-size:12px;margin-top:20px">Auto-generated by GitHub Actions (prebreak-picks). Not investment advice.</p>
</body></html>"""

    msg = MIMEMultipart("alternative")
    msg["Subject"] = f"Pre-Break Picks — {date_str}"
    msg["From"] = smtp_user
    msg["To"] = to_email
