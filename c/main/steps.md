...
第 7 步（记录 + 写次日计划）：用 "<code> <qty> <cost>" 更新 c/main/chold.md，并在次日开盘"前"
...
第 9 步（复盘 + 重复）：收盘后重跑第 1 步并更新 c/main/watchlistd.md（最多 6 只）。
...
试探入场 —— 半仓，只在次日确认后加。
记录 + 写计划 —— c/main/chold.md + 次日指令。
机械管理 —— 止损 / 止盈分批 / 5 日均线移动止损 / 绝不补仓。
复盘 + 重复 —— 重跑闸门，更新 c/main/watchlistd.md，记录违规和错过的入场。
...
流程：
    ...
    c/main/watchlistd.md  -> 选定的 6 只票
    c/main/framed.md      -> 闸门 / 入场 / 管理 / 再入场规则
