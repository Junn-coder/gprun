...
- c/ctool/yao_gate.py — 妖股专用闸门 (池内涨停数 + 指数极端崩盘检测, 不用 swing 系统的 index.py)
...
Rules:
- 闸门来自 yao_gate.py: RED = 池内0涨停或指数>3%暴跌 → 写 "闸门 RED — 空仓" 然后停止
- AMBER = 池内1-2涨停 → 最多选1只; GREEN = 池内3+涨停 → 最多选2只
- 只选昨日涨停 + 非一字板 + 非 ST 的票
- 开盘一字板 (>+9.5%) → 放弃
